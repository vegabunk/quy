...
.css-1lcbmhc .css-1offfwp { margin-bottom: 0.5rem; }
.img-caption { color: black !important; font-weight: bold; }
</style>
...
chap=st.sidebar.radio("Chương",[3,4,9])
file=st.sidebar.file_uploader("Upload ảnh",type=["png","jpg","jpeg","bmp"])
process=st.sidebar.button("Process")

if file:
    img=np.array(Image.open(file).convert('L'))
    st.markdown(f"## Thị Giác Máy – Chương {chap}")
    c1,c2=st.columns(2)
    c1.image(img,caption="Ảnh gốc",use_column_width=True)
    ...
    if process:
        ...
        c2.image(res,caption="Ảnh xử lý",use_column_width=True)
